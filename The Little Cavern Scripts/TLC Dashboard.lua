...
function main()
    ...
    if #sessions == 0 then
        if draw_session_button(curr_y, "Create your home screen now", {100, 150, 100}) then
            local builder_path = reaper.GetResourcePath() .. "TLC Dashboard Setup.lua"
            local cmd_id = reaper.AddRemoveReaScript(true, 0, builder_path, true)
            if cmd_id ~= 0 then
                reaper.Main_OnCommand(cmd_id, 0)
            else
                reaper.ShowMessageBox("TLC Dashboard Setup.lua not found in The Little Cavern Scripts", "Error", 0)
            end
            gfx.quit()
        end
    else
