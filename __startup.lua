...
-- Leer el nombre del estudio configurado en el Session Builder
...
function main()
    ...
    if #sessions == 0 then
        if draw_session_button(curr_y, "Create your home screen now", {100, 150, 100}) then
            local builder_path = reaper.GetResourcePath() .. "/Scripts/jmolas_scripts/Session Builder.lua"
            local cmd_id = reaper.AddRemoveReaScript(true, 0, builder_path, true)
            if cmd_id ~= 0 then
                reaper.Main_OnCommand(cmd_id, 0)
            else
                reaper.ShowMessageBox("Session Builder.lua not found in /Scripts/jmolas_scripts/", "Error", 0)
            end
            gfx.quit()
        end
    else
